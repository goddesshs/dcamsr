...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, out_dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False,
                 img_size=224, patch_size=4, upsample=None):
        ...
        if upsample is not None:
            self.upsample = upsample(self.input_resolution, dim=dim, out_dim = dim)
...
class Conv2D(nn.Module):
    def __init__(self, in_chl, nf, n_blks=[1, 1, 1], act='relu'):
        ...
        block = functools.partial(ResidualBlock, nf=nf, act=act)
        self.conv_L1 = nn.Conv2d(in_chl, nf, 3, 1, 1, bias=True)
        self.blk_L1 = make_layer(block, n_layers=n_blks[0])
